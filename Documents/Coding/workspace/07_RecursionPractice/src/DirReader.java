...
public class DirReader {
	 ...
	 *            starting directory
	 * @param depth
	 *            maximum number of subfolders to list, 0 would be just the
	 *            current file
	 * @return list of all files in directory
	 */
	public List<File> recursiveList(File dir, int depth) {
		...
		for (File current : dir.listFiles()) {
			if (current.isDirectory()) {
				List<File> subFiles = recursiveList(current, depth - 1);
				results.addAll(subFiles);
			} else {// a file
				results.add(current);
			}
	...
	public long recursiveSize(File dir) {
		...
		for (File current : files) {
			if (current.isDirectory()) {
				// get size of items in directory recursively
				sum += recursiveSize(current);
			} else {// a file
				// add size of this file to sum
				sum += current.length();
			}
	 ...
	 *            starting directory
	 * @param level
	 *            specifies the level of indentation
	 */
	...
	/**
	 * Print all files contained in the directory and directories within, with
	 * each file indented by the number of tabs equal to its level
	 * 
	 * @param dir
	 *            starting directory
	 */
	 ...
	 * 
	 * @param count
	 *            specifies how many tabs in returned string
